...
也可以复制仓库中的 `install-runtime.js` 到 Scriptable 新脚本并运行。安装器会从本仓库 `main` 分支下载两个小组件，然后打开 Codex 用量脚本。
...
- 组合小组件接受 `/usage`。
- Claude 小组件接受组合接口 `/usage` 或单服务接口 `/claude-usage`。
...
- 推送 `v*` 标签，例如 `v0.2.0`；
- 在 Actions 页面手动运行 `Release Scriptable Widgets`，输入版本标签。
